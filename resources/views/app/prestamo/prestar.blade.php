...
{!! Form::model($pedido, $formData) !!}	
	...
	{!! Form::hidden('devolver_envases','',array('id'=>'envases_a_prestar')) !!}
									...
									@if($value->producto_id == 4 || $value->producto_id == 5)
										...
										@if( $detalles_devuelto != null)
											@if( $detalles_devuelto[$value->id] != null )
												{!! Form::number($idinput, $detalles_devuelto[$value->id] , array('class' => 'cantidades form-control input-xs', 'readonly','style' => 'width:80px; ', 'id' => $idinput, 'max' => $detalles_prestamos[$value->id], 'min' => '0')) !!}
											@else
												{!! Form::number($idinput, $detalles_prestamos[$value->id] , array('class' => 'cantidades form-control input-xs', 'style' => 'width:80px; ', 'id' => $idinput, 'max' => $detalles_prestamos[$value->id], 'min' => '0')) !!}
											@endif
										@else
											{!! Form::number($idinput, $detalles_prestamos[$value->id]  , array('class' => 'cantidades form-control input-xs', 'style' => 'width:80px; ', 'id' => $idinput, 'max' => $detalles_prestamos[$value->id], 'min' => '0')) !!}
										@endif
